...
for l in range(5):
    ...
    xs_walk = random_walk_metropolis(x_walk, N, p[0], u[0], u0)
    r_xs_theta2 = acf(xs[:, 1])
    ess_xs_theta1 += np.abs(N / (1 + 2 * (acf(xs[:, 0], 1000).sum() - 1)))
    ess_xs_theta2 += np.abs(N / (1 + 2 * (acf(xs[:, 1], 1000).sum() - 1)))
    ess_walk_theta1 += np.abs(N / (1 + 2 * (acf(xs_walk[:, 0], 1000).sum() - 1)))
    ess_walk_theta2 += np.abs(N / (1 + 2 * (acf(xs_walk[:, 1], 1000).sum() - 1)))
print('effective sample size of theta1 for Markov chain generated by simple PT: %f' % (ess_xs_theta1 / 5))
print('effective sample size of theta2 for Markov chain generated by simple PT: %f' % (ess_xs_theta2 / 5))
print('effective sample size of theta1 for Markov chain generated by random walk MH: %f' % (ess_walk_theta1 / 5))
print('effective sample size of theta2 for Markov chain generated by random walk MH: %f' % (ess_walk_theta2 / 5))